...
<body>
  ...
  <div class="container">
    ...
    <form class="add-project__container" method="post"
      action="<?php echo $_SERVER['PHP_SELF']; ?>">
      <input class="form__input" type="text" name="task_name"
        ...
        <?php
                ...
                if ($handle->rowCount() > 0) {
                    while ($getRow = $handle->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $timerTmp = strtotime($getRow['timer']);
                        $timer = date("H:i:s", $timerTmp);
                        if ($getRow['status'] == 1) {
                            $status = 'STOP';
                            $classa = 'class="timer working"';
                        } else {
                            $status = 'START';
                            $classa = 'class="timer not-working"';
                        }
                        $t_id = $getRow['ID'];
                        echo '<form class="projects__row-container" method="post" action="' . $_SERVER['PHP_SELF'] . '">
                          ...
                          <span ' . $classa . ' id="clock"' . $t_id . '">' . $timer . '</span>
                          <span id="project-name">' . $desc . '</span>
                          <input type="submit" name ="button"
                            class="details btn-primary--filled"  value="' . $status . '"/>
                        </form>';
                    }
                }
                ?>
    ...
    <form class="add-project__container" method="POST"
      action="<?php echo $_SERVER['PHP_SELF']; ?>">
      <select class="form__input" name="edit-task" type="text"
        placeholder="Task name">
        <?php
                require_once "connect.php";
                $id = $_SESSION['p_id'];
